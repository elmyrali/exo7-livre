...
\begin{itemize}
  \item Un ensemble $F\subset \Rr^2$ est un \defi{fermé} si toute suite de points $(P_n)_{n\in\Nn}$
...
$\mathbf{f}(C') = C'$. Alors $\dist_H (C,C') \le \max_{i=1,\ldots,\ell} k_i  \cdot  \dist_H (C,C')$.
Comme $\max k_i < 1$ alors $\dist_H (C,C')=0$ et par la proposition \ref{prop:haus}
alors $C=C'$.
...
Nous définissons la \defi{dimension de Hausdorff} d'un ensemble $E$ comme étant le réel $d>0$
vérifiant :
$$\ell=m^d$$
...
Il nous faut en plus un \defi{condition de non-recouvrement} : on suppose qu'il existe un ouvert
...
\begin{exercicecours}[L'ensemble de Cantor]
...
\begin{exercicecours}
Dessiner l'image des shadoks par chacune des transformations
$$\myvec{x}{y} \mapsto \begin{pmatrix}a & b \\ c & d \\  \end{pmatrix}
\myvec{x}{y} + \myvec{e}{f}$$
...
\begin{exercicecours}
Quelles sont les transformations affines qui envoient le shadok debout sur l'autre ?
\begin{center}
\commentfigure{
